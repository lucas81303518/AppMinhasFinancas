...
type
  TF_CadEntradas = class(TF_BaseCadastro)
  ...
  public
    { Public declarations }
    constructor Create(AOwner: TComponent; Documento: TDocumento = nil);
  end;

var
  F_CadEntradas: TF_CadEntradas;

implementation

uses
  UF_CadTipoDeConta, UF_CadFormasPagamento, funcoes, Dmodulo;

{$R *.fmx}

constructor TF_CadEntradas.Create(AOwner: TComponent; Documento: TDocumento);
begin
...
procedure TF_CadEntradas.edtDescricaoEnter(Sender: TObject);
begin
  inherited;
  {$IFDEF ANDROID}
//  foco := TEdit(Sender);
...
procedure TF_CadEntradas.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  try
...
procedure TF_CadEntradas.lblCadastrarFormaPagamentoClick(Sender: TObject);
begin
  inherited;
...
procedure TF_CadEntradas.lblCadastrarTipoDeContaClick(Sender: TObject);
begin
  inherited;
  if not Assigned(F_CadTipoDeConta) then
  begin
    F_CadTipoDeConta := TF_CadTipoDeConta.Create(Self, nil, taSomenteEntrada);
    F_CadTipoDeConta.OnContaIncluida := MontaTipoContas;
    F_CadTipoDeConta.Show;
  end;
end;

procedure TF_CadEntradas.MontaFormasPagamento();
var
...
begin
  ...
  try
//      FListaFormasPagamento := TObjectList<TFormaPagamento>.create;
...
procedure TF_CadEntradas.MontaTela;
begin

end;

procedure TF_CadEntradas.MontaTipoContas();
var
...
begin
  try
    ...
    try
//      FListaTipoContas := TObjectList<TTipoConta>.create;
      FListaTipoContas := FControllerTipoContas.GetPorTipo(1);
    except on Ex: Exception do
      begin
...
procedure TF_CadEntradas.numberBoxValorEnter(Sender: TObject);
begin
  inherited;
  {$IFDEF ANDROID}
...
procedure TF_CadEntradas.recSalvarClick(Sender: TObject);
begin
...
function TF_CadEntradas.ValidarCampos: Boolean;
begin
  Result := False;
